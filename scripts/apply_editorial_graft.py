...
PAGES = {
    "index.html": {
        ...
        "label": "La rupture cognitive",
        "title": "Confondre la compréhension d’un schéma avec sa résolution.",
        "block": "La plupart des personnes qui s’adressent à moi ont une excellente culture d’elles-mêmes. Elles ont lu, analysé, intellectualisé. Elles savent précisément pourquoi elles bloquent. Le piège actuel est là : confondre la compréhension d’un schéma avec sa résolution.",
        "body": [
    ...
    "vision.html": {
        ...
        "label": "L’intelligence relationnelle",
        "title": "La conscience ne libère pas. Seule la structure le fait.",
        "block": "Vous pouvez identifier vos loyautés invisibles pendant des années sans pour autant cesser de les servir. L’intelligence relationnelle ne consiste pas à comprendre le pourquoi, mais à maîtriser le comment.",
        "body": [
    ...
    "territoires.html": {
        ...
        "label": "Les zones de friction",
        "title": "Un mécanisme de défense n’est jamais abstrait.",
        "block": "Il se déploie dans le réel : une réunion où vous vous taisez, une dispute de couple où vous fuyez, une pression familiale où vous vous sur-adaptez. Si vous pensez que ces situations sont des fatalités, vous ignorez le rôle que vous y jouez.",
        "body": [
    ...
    "coaching.html": {
        ...
        "label": "Le laboratoire de test",
        "title": "On ne vient pas pour déposer ses problèmes. On vient pour tester de nouvelles réponses.",
        "block": "Le coaching chez Mirror Me est un laboratoire. Vous ne venez pas raconter votre histoire, vous venez exposer votre automatisme à la lumière pour le neutraliser.",
        "body": [
...
def graft_section(data):
    body = "\n".join(f'        <p>{p}</p>' for p in data["body"])
    return f'''\n\n<section class="section reveal editorial-graft" id="{data['marker']}">\n  <div class="grid-2">\n    <div>\n      <span class="label">{data['label']}</span>\n      <h2 class="t-large">{data['title']}</h2>\n      <div class="editorial-block">\n        <p>{data['block']}</p>\n      </div>\n    </div>\n    <div class="body">\n{body}\n      <p><a class="btn btn-dark" href="{data['href']}">{data['cta']}</a></p>\n    </div>\n  </div>\n</section>\n'''

def insert_after_first_section(html, section):
    m = re.search(r'</section>', html, flags=re.I)
    return html if not m else html[:m.end()] + section + html[m.end():]
...
for filename, data in PAGES.items():
    ...
    if data["marker"] in html:
        skipped.append(f"{filename}: already grafted")
        continue
    if "<nav" not in low or "</nav>" not in low or "<footer" not in low or "</footer>" not in low:
        ...
        continue
    if "...</nav>" in html or "<footer>...</footer>" in html:
        skipped.append(f"{filename}: placeholder detected")
        continue
    original = html
    html = replace_meta(html, data["meta"])
    html = replace_first_h1(html, data["h1"])
    html = insert_after_first_section(html, graft_section(data))
    if html != original:
        path.write_text(html, encoding="utf-8")
        changed.append(filename)

report = "# Editorial graft applied\n\n## Changed\n"
